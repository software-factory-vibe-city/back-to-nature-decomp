...
- A diff that ignores source-level changes (e.g. a commutative operand order identical under both source orders) → some pass is canonicalizing the construct. Find the first `build/compilerTrace/{FUNC_NAME}/` dump where the divergence appears, then read that pass in `notes/scratch/gcc-2.95.2-reference/` before further edits. See "Attribute the rewriting pass before perturbing" in the style guide.
...
4. **Test structural hypotheses side by side** — for an operand-order, allocation, or scheduling mismatch that survives a few traced, mechanism-targeted edits, write each remaining structural hypothesis as a complete variant `.c` (project headers, C89) and run `npx tsx tools/agent/fuzzVariants.ts {FUNC_NAME} <variant1.c> <variant2.c> ...`. The report shows every variant's diff class and first divergence against the original; read it comparatively to identify which web-shape family the compiler preserves. Rules of engagement:
   - This is **hypothesis testing, not match-% hill-climbing**. Do not generate random permutations; each variant must embody a stated mechanism hypothesis (fresh result web vs. reused input, fused expression vs. named temp, struct-field vs. pointer arithmetic, statement order).
   - Promote a winner only after you can name the compiler mechanism it exercises (e.g. "fresh-destination address web defeats cse constant-second canonicalization").
   - `--cc1-only` is fast triage; confirm the finalist in full mode, then copy it over `src/{FUNC_NAME}.c` and re-verify with `diffFunc.ts` and `make check`.
5. **Read the compiler source (escape hatch)** — if an allocation/scheduling mismatch survives several traced, mechanism-targeted edits, stop permuting and consult the vendored GCC 2.95.2 sources (`notes/scratch/gcc-2.95.2-reference/local-alloc.c`, `sched.c`) plus the mechanism table in the style guide ("Escape hatch") and the "Deep allocation/scheduling findings" section above. Identify the exact rule defeating the candidate, then design one source shape that satisfies it. This is diagnostics-only: never patch or instrument cc1.
6. **Scheduling barrier** — only for an independently reordered instruction pair that resists source-order fixes: `__asm__ volatile("" : "=r"(var) : "0"(var));` with a comment stating the exact target-vs-compiler ordering it fixes.
7. **STOP and report** — if these do not work, leave the file at its best clean-C state and report the structural category, trace finding, and remaining instructions. A documented stuck function is valuable; a hacked match is not.
