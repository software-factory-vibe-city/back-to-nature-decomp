...
export function renderSourceShapeSummary(summary: SourceShapeSearchSummary): string {
  const lines = [
    ...
    "",
    `${"variant".padEnd(42)} ${"policy".padEnd(7)} ${"hard".padEnd(5)} ${"requirements".padEnd(14)} ${"schedule".padEnd(12)} ${"mechanism".padEnd(10)} score`,
  ];
  for (const result of summary.results.slice(0, 30)) {
    ...
    const schedule = result.scheduleDelta?.verdict || (result.scheduleAnalysisError ? "inconclusive" : "not-analyzed");
    lines.push(`${result.variantId.slice(0, 41).padEnd(42)} ${(result.policyPassed ? "pass" : "fail").padEnd(7)} ${(result.hardConstraintsPassed ? "pass" : "fail").padEnd(5)} ${requirementText.padEnd(14)} ${schedule.slice(0, 12).padEnd(12)} ${verdict.padEnd(10)} ${result.exactInstructions}/${result.totalInstructions}${result.fullObjectExact ? " OBJECT-EXACT" : ""}`);
  }
  ...
  lines.push("Ranking is lexicographic: policy, hard preservation, requirements, supported schedule delta, mechanism, opcode/count, exact score, object identity.");
