...
FUNC(GsARGUNIT_ANIM *sp0)
{
  ...
  scale.vx = (sp->src->sx*rframe+sp->dst->sx*(tframe-rframe))/tframe;
  scale.vy = (sp->src->sy*rframe+sp->dst->sy*(tframe-rframe))/tframe;
  scale.vz = (sp->src->sz*rframe+sp->dst->sz*(tframe-rframe))/tframe;
