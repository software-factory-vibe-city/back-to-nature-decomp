 ...
 * level : Divide level
 */
static void Adiv_ft4(scratch,ifo,level)
u_long *scratch;
GsADIV_FT4 *ifo;
int level;
{
  register GsADIV_P_FT4 *in,*out;
  ...
  in  = (GsADIV_P_FT4 *)scratch;
  scratch += (sizeof(GsADIV_P_FT4)/4);
  out = (GsADIV_P_FT4 *)scratch;
