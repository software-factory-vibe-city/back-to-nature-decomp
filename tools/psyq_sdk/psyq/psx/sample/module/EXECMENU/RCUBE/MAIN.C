...
#define DISTANCE 600			/* initial value for Radial */
...
main()
{
...
#ifdef CDREAD	/* when CDREAD is not defined, download data ahead of time with PQBLOAD */
	...
	PadStop();
	ResetGraph(3); /* reset with clearing screen */
	StopCallback();
#ifdef DEAD_PARENT
...
void initView()
{
	...
	/* parent coordinates of viewpoints */
	View.super = WORLD;
...
int c_mode;	/* if 0, white light; if 1, coctail lights */
...
void changeTmd()
{
	...
	/* switch color/brightness of light source according to 
	   TMD type */
	switch(CurrentTmd) {
	    case 0:
                /* normal (flat) */
