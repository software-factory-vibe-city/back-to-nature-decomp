...
#define	OT_SIZE		(1<<OT_LENGTH)	/* OT size*/
#define	SCR_Z		1000	/* distance from view point to the screen*/

#define OBJECTMAX 100		/* Max Objects defines the maximum number of logical objects a 3D model can be broken down into  */
...
main()
{
	GsCOORDINATE2	objWorld;	/* object coordinate */
	int		objcnt;
	DB		db[2];		/* packet double buffer */
	int		idx;		/* current db index */
	SVECTOR		rotate;		/* object rotation vector*/
...
static void init_view()
{
	...
	/* set position for projection surface*/
...
static void init_light()
{
	...
	/* set light-source mode (standard light source/no FOG) */
	GsSetLightMode(0);
}
