...
/*
 *
 *        Physics simulation program using fixed point numbers
 *
...
texture_init(addr)              /* load texture data to VRAM*/
u_long addr;
{
  RECT rect1;
  GsIMAGE tim1;

  GsGetTimInfo((u_long *)(addr+4),&tim1); /* get texture data type from TIM data header*/
  rect1.x=tim1.px;               /* X coordinate in VRAM of upper left texture point*/
  rect1.y=tim1.py;         /* Y coordinate in VRAM of upper left texture point*/
  rect1.w=tim1.pw;              /* texture width*/
  rect1.h=tim1.ph;              /* texture height*/

  LoadImage(&rect1,tim1.pixel); /* load texture to VRAM*/
  tpage=GetTPage(tim1.pmode,0,640,0);
  tpage= tpage&0xfe7f;		/*for GetTPage bug*/

  if((tim1.pmode>>3)&0x01)      /* color look-up table exists*/
    {
      rect1.x=tim1.cx;          /* X coordinate in VRAM of upper left CLUT point*/
      rect1.y=tim1.cy;          /* Y coordinate in VRAM of upper left CLUT point*/
      rect1.w=tim1.cw;          /* CLUT width*/
      rect1.h=tim1.ch;          /* CLUT height*/
      LoadImage(&rect1,tim1.clut); /* load CLUT to VRAM*/
    }
