...
{				/* set up the modeling data */
  ...
  for(i=0,objp=object;i<Objnum;i++)
    { /* default object coordinate */
      ...
      /* objp->attribute |= GsLOFF;	/* light caliculation off */
...
/* Set the minimum lower functions which is called by GsSortObject5J() */

/* In this case only flat triangle with lighting set the jump table */
