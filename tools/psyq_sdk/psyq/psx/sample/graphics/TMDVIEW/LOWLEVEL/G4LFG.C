...
/* 
 *	PolyG4 packet sorting routine	(FOG on)
 *
...
PACKET *FastG4LFG(op, vp, np, pk, n, shift, ot)
	TMD_P_G4 *op;
	VERT   *vp, *np;
	PACKET *pk;
	int     n, shift;
	GsOT   *ot;
{
	...
	for (i = 0; i < n; i++, op++) {
		...
		/* finds normal colors from normal vectors, performs depth queing */
		NormalColorDpq3((SVECTOR *)&np[op->n0], (SVECTOR *)&np[op->n1], 
				(SVECTOR *)&np[op->n2], (CVECTOR *)&op->r0, p,
				(CVECTOR *)&si->r0, (CVECTOR *)&si->r1, (CVECTOR *)&si->r2);
		NormalColorDpq((SVECTOR *)&np[op->n3], (CVECTOR *)&op->r0, p,
				(CVECTOR *)&si->r3);
