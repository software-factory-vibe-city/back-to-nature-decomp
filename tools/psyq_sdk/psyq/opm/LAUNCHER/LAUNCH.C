...
AppInfo DemoApps[NUM_APPS] = {
  ...
  {"\\MENU\\MAIN.EXE;1", 24, 0, 23},
   /* Shadow sample - Child 1*/
  {"\\SHADOW\\SHADOW.EXE;1", 41, 0, 40},
   /* Hi-Res 60 fp balls - Child 2 */
  {"\\FBALLS\\FBALLS.EXE;1", 70, 0, 69},     
   /* tdor3 - Child 3 */
  {"\\TDOR3\\TDOR3.EXE;1", 93, 0, 92},     
...
int ExecMode = MODE_INTERACTIVE; // This is the mode that your program will
                                 // be launched in. 
int CurrentApp = 0;   // launch the menu first!
int LastApp = 0;
int FirstTime = 1;
...
void GoProg(int app)
{
  ...
  if (app != 0) {                /* this is not the menu app, 
