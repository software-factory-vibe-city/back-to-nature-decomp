...
/* Read the WITHOUT-unknown CSV: the --disasm-unknown split turns data
 * regions into phantom func_ entries, leaving no T_ data entries to build
 * regions from. */
const CSV_PATH = join(ROOT, "build/without-unknown/functions.csv");
const FUNCTIONS_DIR = join(ROOT, "build/functions");
const OUTPUT_PATH = join(ROOT, "notes/access-patterns.md");
const JSON_OUTPUT_PATH = join(ROOT, "build/accessRegions.json");
const GP_VALUE = EXE.gpValue;
...
/* Machine-readable region table for downstream tools (bootstrap.ts uses it
 * to set sectionLayout.sdataStart — the only honest .sdata signal, since
 * position-in-GP-range is not sufficient). */
const jsonRegions = analyses.map((a) => ({
  startAddr: a.region.startAddr,
  endAddr: a.region.endAddr,
  section: a.inferredSection,
  confidence: a.confidence,
  symbolCount: a.symbols.length,
  gpRelative: countSymsWith(a.symbols, (s) =>
    s.accessTypes.has("gp_read") || s.accessTypes.has("gp_write")
  ),
}));
writeFileSync(JSON_OUTPUT_PATH, JSON.stringify(jsonRegions, null, 1) + "\n");
