...
/*
 * Divide Gouraud Quad (GsUDG4)
 */
u_long *GsU_00010014(GsARGUNIT *sp)
{
	...
	divp->piv = VWD0;
  	...
  	for (i = 0; i < num; i++, tp++) {
		...
		if (RotAverageNclip4(&r0->v, &r1->v, &r2->v, &r3->v,
			...
			NormalColorCol3(np+tp->n0, np+tp->n1, np+tp->n2,
				&tp->r0, &r0->c, &r1->c, &r2->c);
			NormalColorCol(np+tp->n3, &tp->r0, &r3->c);
