...
int level;
{
  ...
  in  = (GsADIV_P_GT4 *)scratch;
  scratch += (sizeof(GsADIV_P_GT4)/4);
  out = (GsADIV_P_GT4 *)scratch;
  ...
  *(u_short *)&si->u2 = *(u_short *)&in->vt[2].tu;
  ...
  gte_stsxy3_gt4((long *)si);
  
  gte_ldv0(&in->vt[3].vx);
  gte_rtps();
  *(u_short *)&si->u3 = *(u_short *)&in->vt[3].tu;
  gte_stflg(&ifo->flg0);
  ifo->flg |= ifo->flg0;
...
/*
static int clip_sz(ifo)
GsADIV_GT4 *ifo;
{
  gte_stsz4(&ifo->otz0,&ifo->otz1,&ifo->otz2,&ifo->otz3);
  if(ifo->otz0==0 && ifo->otz1==0 && ifo->otz2==0 && ifo->otz3==0)
    return 1;
  else
    return 0;
}
