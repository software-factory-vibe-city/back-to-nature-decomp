 ...
 * level : Divide level
 */
static void Adiv_g4(scratch,ifo,level)
u_long *scratch;
GsADIV_G4 *ifo;
int level;
{
  register GsADIV_P_G4 *in,*out;
  register POLY_G4 *si;
  static void sukima();
  static int outline();
  static void bunCV();
  
  in  = (GsADIV_P_G4 *)scratch;
  scratch += (sizeof(GsADIV_P_G4)/4);
  out = (GsADIV_P_G4 *)scratch;
  ...
  if(ifo->flg>=0 && ifo->maxx-ifo->minx<ifo->adivw &&
    ...
    aaa:
      ...
      *(ifo->pk+7) = *(u_long *)&in->vt[3].col.r;
      *(ifo->pk+8) = *(u_long *)((u_long *)si+8);
      ifo->tag = ifo->org+(ifo->otz>>ifo->shift);
      *(u_long *)ifo->pk  = (*ifo->tag&0x00ffffff)|0x08000000;
      *ifo->tag = (u_long)ifo->pk&0x00ffffff;
      ifo->pk += sizeof(POLY_G4)/4;
      return;
    }
