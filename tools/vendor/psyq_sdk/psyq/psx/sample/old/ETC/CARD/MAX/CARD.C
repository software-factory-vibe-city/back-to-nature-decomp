...
void funcMemCard( long pad )
{
    ...
    /* do initializing at the only first time */
    if (openingflag == 0) {
  	if (openingcnt == 0) {
	    funcInitCard(0);
	    funcInitCard(1);
    ...
    /* register the sprite packet of ICON */
    ...
    /* the case of being specified both of the operation and ICON */
    if( Event && Target ) {
#ifdef DEBUG
...
//////////////////////////*/
/*
  read the file name from Memory Card    FileList[side][].name
  store the number of block              FileEntry[side]
  store the number of file               FileEntry[side] */
static void	funcInitCard(int side)
{
    struct	DIRENTRY d;
    char	flist[8];
...
//
//////////////////////////*/
/* the operation while selecting ICON */
static int	funcMove(int side)
{
    ...
    cardmv[side].flag = 0;
    /* (re)initialize the image of ICON */
	...
	if (GetCardInf(flist, &CardInf[side][n])) {
	    /* can't read the Header informations */
	...
	if ((CardInf[side][n].BlockEntry <= 0) || 
		...
		/* the case of illegal block number */
#ifdef DEBUG
	...
	/* read the number of Block FileList[side][].bnum */
...
//
//////////////////////////*/
/* initialize the sprite for the text name */
static void funcStringTim(side)
{
	...
	/* get the u,v value of polygon of the text name */
	...
	if (side == 0) {
	    /* copy the polygon of the text name to Sprites[fot][] */
	    spriteCount += 2;	
...
//
//////////////////////////*/
/* callback routine for button of Memory Card */
void	funcMenu( long pad )
{
  int	id;
  int     i;
...
static int DeleteFileList(int side)
{
  ...
  for (i = 1; i < 16; i++) {
	...
	/* shift the Deleted FileList[side][], and store NULL */
	for (j = i; j < 15; j++)
...
static int CheckFileList(char *fnam, int side)
{
  ...
  for (i = 1; i < 16; i++) {
      /* search the FileList[side][].name by fname OK : -1, NG : 0 */
#ifdef DEBUG1
...
//
//////////////////////////*/
/* clear the sprite for the text name */
void SetStringUVclr(int flag)
{
    if( flag == 0 )
      SetStringUV(flag);
...
//
//////////////////////////*/
/* replace the sprite for the the text name */
void SetStringUV(int current)
{		
    int	side;
    int	rside;
	...
	/* get the current ICON number FileList[side][].sprtnum */
    ...
    /* re-generate sprites if top address of the sprite isn't same as side */
    /* re-generate sprites if card was pull out */
#ifdef EEMOUSE
...
printf("unuseTex: (x,y,w,h)=(%d,%d,%d,%d)\n", rect.x, rect.y, rect.w, rect.h);
	    ...
	    for (i = 0; i < 16; i++) {
	    /* the case of ICON of opposite side, re-set up the u,v of 
		FileList[side][] */
    ...
    /* copy the current text name to StrSprites[fot][] */
...
/* called by the main loop in max.c */
void CopyMemCard()
{
    int ret;
    int side;
...
void MemCardCopy()
{
  ...
  int	cpwidth;
	...
	return;
      ...
      /* copy the FileList[side][] to the another side (rside) */
