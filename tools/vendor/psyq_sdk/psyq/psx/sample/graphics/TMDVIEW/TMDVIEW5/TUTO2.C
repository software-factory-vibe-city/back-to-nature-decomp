  ...
  while(1)
    {
      if(obj_interactive()==0)
        return 0;	/* get motion parameters from pad data */
      ...
      /* Clear OT2, object2 always exists in front of object1 because
	 OT2's point value is 0 */
      GsClearOt(0,12,&Wot2[outbuf_idx]);
      /*
	Clear OT2, object2 always lays behaind object1 because OT2's point
	value is set to 1<<OT_LENGTH-1 */
      for(i=0,op=object;i<Objnum;i++)
	{			/* object1 caliculation */
