 ...
 *
 *  1995 Sep,1 (C) Sony Computer Entertainment Inc.
 *
...
/*  ver3.2 later libgs.h including VERTC
typedef struct {
  short   vx,vy,vz;
  u_char  tu,tv;
  CVECTOR col;
 ...
 */
u_long *sort_tg_plane(pk,shift,ot,scratch)
PACKET *pk;
int  shift;
GsOT   *ot;
u_long *scratch;
{
  ...
  ifo->limit    = 4;		/* recursive call max limit */
  ...
  ifo->nearz    = Projection/2;
  setPolyGT4(&ifo->si);
  
  /* quad polygon parameter difinition */
  set_xyzuvrgb(p1->vt[0],-1000,-1000,1000,0,0,0x80,0x80,0x80);
  set_xyzuvrgb(p1->vt[1],1000,-1000,1000,255,0,0x80,0x80,0x80);
  set_xyzuvrgb(p1->vt[2],-1000,1000,1000,0,255,0x80,0x80,0x80);
  set_xyzuvrgb(p1->vt[3],1000,1000,1000,255,255,0x80,0x80,0x80);
