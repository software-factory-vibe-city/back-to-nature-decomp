 ...
 * - When starting up specific sample functions for examination, specify the fun
 ...
 *   for each function.
 * - Note that calling initialization functions for unused libraries will increa
 *   code size.
 ...
 *
 * Description      Records input history of the controller with myPadInit(1) an
 *        plays back the same input using myPadINit(2).
 *
 * Return value     none
 *   
 * Notes       Memory contents recorded with myPadInit(1) are temporarily saved
 *        to a file so that the same input operations can be repeatedly played b
 ...
 *
 * Description      Used in conjunction with myPadInit(). See the section on Pad
 *
