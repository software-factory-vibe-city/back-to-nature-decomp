/*
 *           Download Tool for Indy -> PlayStaion
 *           sbload by izumi
 *
...
main(int argc, char **argv)
{
  ...
  if(argc != 3) {
    printf("Usage: sbload <file> <address>\n");
    exit(1);
  }
...
/*  printf("#%d chlen=%d sum=%02x sts=%x\n", i, chlen, sum, sts&0xff);*/
    if(sts==PCKT_SUCCESS) i++;	/* success packet sending ? */
  }
...
/*  printf("chlen=%d sum=%x\n", chlen, sum);*/
  for(i=0, timeout=0; i<chlen; i++, timeout++) {
    ...
    if(timeout==TIMEOUT_MAX) {
      printf("Cannot write data to serial port.\n");
      fclose(fp);
      close(fs);
      exit(1);
    }
